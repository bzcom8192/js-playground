...
import SmallDownloadIcon from "./components/icons/SmallDownloadIcon";
import TrashIcon from "./components/icons/TrashIcon";
import XIcon from "./components/icons/XIcon";
import PlayIcon from "./components/icons/PlayIcon";

const MonacoEditor = lazy(() => import("./components/MonacoEditor"));
const SmartString = lazy(() => import("./components/SmartString"));
const ObjectInspector = lazy(() => import("./components/ObjectInspector"));
const NumberToggle = lazy(() => import("./components/NumberToggle"));
const BooleanToggle = lazy(() => import("./components/BooleanToggle"));
...
export default function App() {
  ...
  const executeCode = useCallback(() => {
    ...
    const mockConsole = {
      log: (...args) =>
        setLogs((p) => [
          ...p,
          { type: "log", parts: processArgs(args), id: Math.random() },
        ]),
      warn: (...args) =>
        setLogs((p) => [
          ...p,
          { type: "warn", parts: processArgs(args), id: Math.random() },
        ]),
      error: (...args) =>
        setLogs((p) => [
          ...p,
          { type: "error", parts: processArgs(args), id: Math.random() },
        ]),
      info: (...args) =>
        setLogs((p) => [
          ...p,
          { type: "info", parts: processArgs(args), id: Math.random() },
        ]),
      clear: () => setLogs([]),
    };
  ...
  const handleLoadSnapshot = (snapshot) => {
    if (
      confirm(`Replace current code with snapshot from ${snapshot.timestamp}?`)
    ) {
  ...
  const handleExportCurrentSession = () => {
    ...
    exportFile(
      data,
      `session-current-${new Date().toISOString().slice(0, 10)}.json`,
    );
  };
      ...
      try {
        ...
        if (data.type === "js-playground-session") {
          ...
          if (isFullBackup) {
            if (
              confirm(
                "This file contains a full history backup. Do you want to overwrite your entire history and current code?",
              )
            ) {
              setCode(data.code || "");
            ...
            if (
              confirm(
                "Import this session? It will replace your current code workspace.",
              )
            ) {
              setCode(data.code || "");
            }
          ...
          if (
            confirm(
              "File doesn't look like a playground session. Import as raw text code?",
            )
          ) {
    ...
    <div className="flex flex-col h-screen bg-gray-950 text-gray-200 font-sans relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
              {history.length === 0 ? (
                <div className="text-gray-500 text-sm text-center mt-10 p-4">
                  No snapshots saved.
                  <br />
                  Click the Save icon in the header to keep your work.
                </div>
              ) : (
                history.map((item) => (
                  <div
      ...
      <header className="flex items-center justify-between px-6 py-3 bg-[#1e1e1e] border-b border-gray-800 shadow-xl z-10 h-16">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-white text-lg shadow-blue-900/50 shadow-lg">
            JS
          </div>
          <h1 className="text-lg font-bold text-gray-100 tracking-tight hidden sm:block">
            Playground
          </h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => setLogs([])}
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-400 hover:text-red-400 transition-colors"
            title="Clear Console"
          >
            <TrashIcon />
          </button>
          ...
          <button
            onClick={executeCode}
            className="flex items-center gap-2 px-5 py-2 text-sm font-bold text-white bg-blue-600 rounded-md hover:bg-blue-500 hover:shadow-lg hover:shadow-blue-900/30 transition-all active:scale-95"
          >
            <PlayIcon /> Run
          </button>
        </div>
        ...
        <div className="flex-1 flex flex-col border-r border-gray-800 min-h-[50%] md:min-h-auto relative group">
          <div className="px-4 py-2 bg-[#1e1e1e] text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-800 flex justify-between select-none">
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={handleReset}
                className="text-[10px] text-gray-500 hover:text-red-400 normal-case transition-colors"
              >
                (Reset)
              </button>
            </div>
        ...
        <div className="flex-1 flex flex-col bg-[#0d1117] min-h-[40%] md:min-h-auto">
          ...
          <div className="flex-1 overflow-y-auto p-4 font-mono text-sm space-y-2 custom-scrollbar">
            ...
            {logs.map((log) => (
              <div
                key={log.id}
                className={`relative pl-2 border-l-2 py-1 wrap-break-word font-medium group/line
                ${
                  log.type === "error"
              ...
              >
                <span
                  className={`inline-block mr-2 opacity-60 select-none font-bold align-top
                  ${
                    log.type === "error"
                      ? "text-red-500"
                      : log.type === "warn"
                ...
                <div className="inline-block align-top break-all">
                  {log.parts.map((part, i) => (
                    <span key={i} className="mr-2 last:mr-0 inline-block">
                      <Suspense fallback={<span>{part.value}</span>}>
                        {part.type === "number" ? (
                          <NumberToggle value={part.value} />
                        ) : part.type === "boolean" ? (
                          <BooleanToggle value={part.value} />
                        ) : part.type === "string" ? (
                          <SmartString value={part.value} />
                        ) : part.type === "object" ? (
                          <ObjectInspector value={part.value} />
                        ) : part.type === "null" ? (
            ...
            {error && (
              <div className="mt-2 p-3 bg-red-950/30 border-l-4 border-red-500 text-red-400 wrap-break-word shadow-sm text-xs md:text-sm">
